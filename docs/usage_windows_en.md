...
- **MCP backend**: `serial_mcp_server.exe` (where you installed or extracted it)
...
2. **Open “MCP Config && Claude Integration”**  
   - **Backend**: **C++** and the full path to **`serial_mcp_server.exe`**.  
...
Set `command` to the real path of **`serial_mcp_server.exe`** on your PC:

```json
{
  "mcpServers": {
    "serial": {
      "command": "C:\\Path\\To\\serial_mcp_server.exe",
      "args": [],
      "env": {
